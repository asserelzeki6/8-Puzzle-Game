    ...
    def run(self):
        ...
        while True:
            if limit>=max_depth:
                break
            frontier = []
            frontier.append((self.start_state,0))
            came_from = {}
            visited = {self.start_state :0}
            while frontier:  # Loop while the frontier is not empty
                current,depth = frontier.pop()  # Dequeue the next state to explore
                ...
                if depth < limit:
                    visited[current] = 0  # Mark the state as visited
                    ...
                    for next_state in self.get_neighbors(current):
                        if next_state not in visited:  # Process only unvisited states
                            frontier.append((next_state,depth+1))  # Add the neighbor to the frontier
                            came_from[next_state] = current  # Record where we came from
                            self.search_depth = max(self.search_depth, depth+1)  # Update search depth
            limit+=1
